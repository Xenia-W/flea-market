...
<script>
  ...
  export default {
            ...
            render: (h, params) => {
              return h('div', [
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.sendout(params.row.orderId,params.index)
                    }
                  }
                }, '确认发货'),
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.remove(params.row.orderId, params.index)
                    }
                  }
                }, '取消')
              ])
    ...
    created: function () {
      ...
      console.log('********* 查询正在出售: call *****************')
      var to = global.contractAddress
      var funcName = "getSalingRecord"
      var para = ""
      console.log(para)
      window.postMessage({
      ...
      window.addEventListener('message', function(e) {
        ...
        if(curPath !== '/salenow')
          return
        // console.log("recived by 查询正在出售:" + e + ", e.data:"+ JSON.stringify(e.data))
        if(!!e.data.data.neb_call){
          ...
          var orderHistory = result.split('#')
          for (var i = 0; i < orderHistory.length; i++) {
            try {
              var tmpOrder = JSON.parse(orderHistory[i])
    ...
    methods: {
      sendout: function(orderId,index) {
        console.log('确认发货的orderId:' + orderId)
        var tmp = this

        console.log('********* 确认发货: sendTransaction *****************')
        var to = global.contractAddress
        var funcName = "orderSendout"
        var para = "[\"" + orderId + "\"]"
        console.log(para)
        window.postMessage({
          "target": "contentscript",
      ...
      remove: function (orderId,index) {
        console.log('取消订单id:' + orderId)
        var tmp = this

        console.log('********* 卖家取消订单: sendTransaction *****************')
        var to = global.contractAddress
        var funcName = "salerOrderCancel"
        var para = "[\"" + orderId + "\"]"
        console.log(para)
        window.postMessage({
          "target": "contentscript",
